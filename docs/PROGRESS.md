...
| 09B | Member resolver, `list_family_members`, assignment read models | `services/members.py`, `familywall/calendar.py`, `familywall/lists.py`, `tools/registry.py`; [handoff](handoffs/09b-member-resolver.md) | **done 2026-09-25**: offline, unit-tested (449 passing); live-verified 2026-09-25 (read-only parse of real events and list items; `list_family_members` in slice G) |
| — | **Brief 09 slice D — generalized receipts and in-place SQLite migration** (`resource_id`/`action`/`rejected`, legacy-schema migration, `create_calendar_event` records refusals as `rejected`) | `models.py`, `storage/sqlite.py`, `services/lists.py`, `services/calendar.py`; [handoff](handoffs/09d-receipts-migration.md) | **done, offline**: 21 unit tests added/updated (R1–R14), full check suite green |
| 09C | Attendees (`assigned_to`, default everyone) and the default reminder on `create_calendar_event` | `familywall/calendar.py`, `services/calendar.py`, `services/principal_context.py`, `server.py`, `tools/registry.py`; [handoff](handoffs/09c-calendar-attendees.md) | **done**: 488 passing (C1–C15); live-verified 2026-09-25 (everyone and named modes both `confirmed`, reminder read back, events deleted) — next: slice E |
| 09E | `set_calendar_event_attendees`: narrow attendees-only update, refusing unsafe events before any write | `familywall/calendar.py`, `services/calendar.py`, `tools/registry.py`; [handoff](handoffs/09e-set-event-attendees.md) | **done**: 524 passing (E1–E13); live-verified 2026-09-25 in slice G |
| 09F | List assignment: single-call `taskcreate2` `add_list_item` (supersedes create-then-move, [ADR 0003](decisions/0003-single-call-add.md)) and the new `set_list_item_assignees` | `familywall/lists.py`, `services/lists.py`, `models.py`, `tools/registry.py`; [handoff](handoffs/09f-list-assignment.md) | **done**: 518 passing (F1–F14); live-verified 2026-09-25 in slice G, including everyone on `taskcreate2` |
| 09G | Live acceptance of every brief 09 tool | [handoff](handoffs/09g-live-acceptance.md) | **done 2026-09-25**: all new and changed tools `confirmed` on disposable data; nothing left behind |
