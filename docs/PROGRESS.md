...
| 09E | `set_calendar_event_attendees`: narrow attendees-only update, refusing unsafe events before any write | `familywall/calendar.py`, `services/calendar.py`, `tools/registry.py`; [handoff](handoffs/09e-set-event-attendees.md) | **done, offline**: 524 passing (E1–E13) — next: slice G live check |
| 09F | List assignment: single-call `taskcreate2` `add_list_item` (supersedes create-then-move, [ADR 0003](decisions/0003-single-call-add.md)) and the new `set_list_item_assignees` | `familywall/lists.py`, `services/lists.py`, `models.py`, `tools/registry.py`; [handoff](handoffs/09f-list-assignment.md) | **done, offline**: 517 passing (F1–F14); not yet live-verified — next: the lead's live check (slice G), including everyone on `taskcreate2` |
