...
| `add_list_item` | title, optional family/list IDs, optional `assigned_to` member names (blank means everyone), operation ID | Created directly in the chosen list, already assigned, in one `taskcreate2` call (no separate move step, see [ADR 0003](decisions/0003-single-call-add.md)); confirmed, misfiled, mismatched, acknowledged or unknown from an exact-ID readback |
| `set_list_item_checked` | list/item IDs, boolean checked, operation ID | Confirmed target state, no toggle |
| `set_list_item_assignees` | item ID, optional `assigned_to` member names (blank means everyone), operation ID | Changes only who the item is assigned to, after verifying the item belongs to an accessible list; confirmed only when the readback shows the new assignment and every other field unchanged |
...
| `set_calendar_event_attendees` | event ID, the event's local date, optional `assigned_to` member names (blank means everyone), operation ID | Changes only the attendees of an existing timed, non-recurring, editable event on the family calendar; refuses before any write if the event is on another calendar, not editable, recurring, a series exception, not an ordinary event, or all-day; confirmed only when a readback shows the new attendees and every other field unchanged |
